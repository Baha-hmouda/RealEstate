...
const SeeAllHouses = ({ navigation }) => {
  ...
  const toggleFavorite = useCallback(async (houseId) => {
    ...
    try {
      const result = await axios.post(`http://192.168.1.201:4000/api/favorite/toggle`, { userId, estateId: houseId, type: 'house' });
...
const getUserId = async () => {
  ...
  const fetchHouses = async (page) => {
    ...
    try {
      const response = await axios.get(`http://192.168.1.201:4000/api/house/allhouses?page=${page}&limit=15`);

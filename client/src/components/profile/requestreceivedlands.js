...
const RequestsList = () => {
  ...
  const fetchRequests = async (userId, savedStatuses = {}) => {
    if (!userId) return;
    try {
      const response = await axios.get(`http://192.168.1.201:4000/api/request/seller/${userId}/land`);
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.post(`http://192.168.1.201:4000/api/request/updateLandRequestStatus/${id}`, { status });
